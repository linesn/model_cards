...
Laguna M.1-NVFP4 is a 225B total parameter Mixture-of-Experts model with 23B activated parameters per token designed for agentic coding and long-horizon work. This is the NVFP4-quantized variant of [Laguna M.1](https://huggingface.co/poolside/Laguna-M.1).

> [!NOTE]
> This is the NVFP4 variant. The [BF16](https://huggingface.co/poolside/Laguna-M.1) and [FP8](https://huggingface.co/poolside/Laguna-M.1-FP8) variants are also available on Hugging Face.
...
- Quantization: NVFP4 (weights), detected automatically from `quantization_config`
...
The full vLLM recipe is on the main [Laguna M.1 model card](https://huggingface.co/poolside/Laguna-M.1). Quantization is detected automatically from `quantization_config` in this checkpoint, so the same command works with `poolside/Laguna-M.1-NVFP4` substituted for the model ID. No extra flags required.
