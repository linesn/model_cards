...
Antares-350M is an open-weight, 350-million parameter language model specialized for vulnerability localization in real-world codebases. Built on IBM Granite 4.0 350M, it is trained to autonomously navigate source code repositories through a terminal interface — issuing shell commands, reading files, and iteratively narrowing in on vulnerable files — using a two-stage pipeline: supervised fine-tuning (SFT) followed by Group Relative Policy Optimization (GRPO).

Unlike traditional static analysis tools or retrieval-based approaches, Antares-350M operates as a terminal agent: it explores repositories by executing commands (`grep`, `find`, `cat`, and standard Unix utilities), reasons about the output, and submits a list of files it believes to contain the reported vulnerability.

Antares-350M is part of the Antares family of models (350M, 1B, 3B), all built on IBM Granite 4.0 and trained with the same two-stage pipeline. The 350M model achieves a File F1 of **0.135** on the Vulnerability Localization benchmark (VLoc Bench), outperforming models many times its size including Gemini 2.5 Flash, Gemma-4-31B, GPT-5 Mini, and Qwen3.5-122B-A10B.

> [!NOTE]
> Within the family, Antares-350M can serve as a speculative draft model for the larger Antares variants in compatible deployments: it proposes candidate tokens that a 1B or 3B model verifies, accelerating inference while preserving the larger model's localization quality. It can also run standalone in resource-constrained deployments.

Antares enables organizations to deploy compact, on-premises vulnerability localization capabilities without relying on cloud-based AI services. In our evaluation setup, Antares-350M completes the full 500-task VLoc Bench sweep in approximately 11 minutes on a single H100 GPU using 16 parallel workers.

- **Model Name:** Antares-350M
...
- **Model Architecture:** Auto-regressive decoder-only transformer (IBM Granite 4.0 350M backbone): 28 layers, hidden dim 1024, 16 attention heads, 4 KV heads (GQA), 32K context window, 100,352 vocab, SwiGLU activation, RMSNorm, RoPE positional encoding.
...
Antares-350M is designed for security practitioners, researchers, and developers building automated vulnerability-localization workflows. The model is optimized for:
...
- **Shift-Left Security**: Integrating into CI/CD pipelines to surface vulnerable files early in the development lifecycle. The 350M model runs on a single GPU, making it practical for automated pipeline use.
- **Advisory-Driven Triage**: Using CWE identifiers and generic category descriptions, including those mapped from CVE or GHSA records, to guide targeted, multi-step repository exploration that narrows down candidate files for analyst review.
...
Antares-350M can be integrated into security toolchains as a terminal-based agentic component. Example downstream applications include:

- **Vulnerability Search**: Locating vulnerable files from CWE identifiers and generic category descriptions by issuing shell commands against a repository snapshot
- **Security Workflow Integration**: Augmenting static analysis tools with reasoning-driven repository exploration; using CWE mappings derived from CVE or GHSA records to surface relevant code; supporting security analysts in evidence gathering and hypothesis refinement
...
For questions or assistance with integrating Antares-350M, please contact Hadas Birin ([**hbirin@cisco.com**](mailto:hbirin@cisco.com)).
...
tokenizer = AutoTokenizer.from_pretrained("fdtn-ai/antares-350m")
model = AutoModelForCausalLM.from_pretrained("fdtn-ai/antares-350m")
...
Antares-350M was trained through a two-stage pipeline:
...
Antares-350M is based on the **IBM Granite 4.0 350M** architecture. Training was performed on Cisco Foundation AI's internal compute cluster.
...
Antares-350M was evaluated on the Vulnerability Localization Benchmark (VLoc Bench), which comprises 500 tasks drawn from 290 unique real-world repositories, spans 6 package ecosystems and 147 unique CWE categories, and includes assigned CVE identifiers for 78% of tasks. Each task pairs a vulnerable repository snapshot with ground-truth vulnerable implementation files; tests, documentation, and configuration files are excluded from the label set.

The agent receives a CWE identifier and its generic category description — no advisory text, file hints, or severity details — and must explore the repository within a budget of 15 terminal commands per task, followed by one final submission action.

**Metric calculation:** For each task, submitted file paths are compared with the ground-truth set of vulnerable implementation files. Precision is the fraction of submitted files in the ground-truth set, recall is the fraction of ground-truth files submitted, and File F1 is the harmonic mean of task-level precision and recall. A task with no valid file submission receives zero precision, recall, and File F1 because every Phase A task contains a known vulnerability. We macro-average precision, recall, and File F1 across all 500 tasks, then average the resulting metrics across three independent runs. The reported File F1 is therefore the mean task-level File F1 across all tasks and runs.
...
| Granite 4.0 350M (base) | 350M | 0.001 | — | — |
...
Antares-350M has several limitations that users should be aware of:
...
4. **No intended exploitation functionality:** The model is not designed or evaluated to generate proof-of-concept exploits or provide vulnerability explanations. It outputs candidate file paths for human review; its internal reasoning guides the search but is not a substitute for a validated security explanation.
...
To address the limitations of Antares-350M, we recommend:
