...
[![Foundry](https://img.shields.io/badge/Azure-Foundry-0089D6)](https://aka.ms/fara1.5-27B-foundry)
...
| **Training compute** | 64 × NVIDIA B200, 6 days |
...
The safest way to run Fara1.5-27B is inside **MagenticLite**, which provides:
...
- `transformers >= 5.2.0`
- `vllm >= 0.19.1`
- A set of GPUs with enough memory for a 27B model in bf16 (A6000, A100, H100, and B200 have been tested). We recommend sharding the model over at least 2 GPUs.
...
Fara1.5-27B emits actions as `<tool_call>...</tool_call>` XML blocks containing a JSON object that calls the `computer_use` function. Supported actions:
...
Fara1.5-27B is trained to **pause and ask the user** at three types of critical points:
...
Developers deploying Fara1.5-27B should apply responsible AI best practices and ensure compliance with applicable laws and regulations. Using safety services like [Azure AI Content Safety](https://azure.microsoft.com/en-us/products/ai-services/ai-content-safety/) is recommended.
